...
contract SmartFruit {   
  ...
  //---------- 参数定义 ------------------------------------------------------
  //开发者账户
  address constant DEVELOPER_ADDRESS = 0xADBCcdd7578F391724836F54e9927c81b9009F64;
...
*/  
  ...
  // 判断水果数量与金额是否一致
  modifier validMoney(uint _num) {
    require(_num * FRUIT_PRICE <= msg.value, 'Invalid num, insufficient value');
    _;
  }
  ...
  function login_UI(address addr) public returns(uint ret){
      ...
      if(u.state==4){
          if( u.expirationDate < now && u.id != 1 ){
              u.state=0;
          }  
      }	
  ...
  function buyFruit_UI(address addr, uint8 count) public payable userRegistered() validMoney(count) {
      User storage u = users[addr];
      u.fruit += count;
    //   if(isFreezing(u)){      
  ...
  //提取现金，把可用现金转到用户账户   
  function withdrawcCash_UI(address addr) public userRegistered() validAddress(addr) returns(bool) {
    //User storage u = users[addr];
    Account storage ac = accounts[addr];
    if(ac.cash==0){
      //用户无效/可用现金为0， 提取失败
  ...
  function buyFruitPayment(address addr, uint value) private {  	  	
    ...
    for(uint8 j=1; j<=12;j++){
      toHigherPayment(j, addr, value*levelPercent[j]/100, addr);
    }
  ...
  // 向对应上级转账，共12级；如果上级无效或冻结，就跳过继续向上一级。_startUser存放初始来源值。
  function toHigherPayment(uint _level, address _user, uint value, address _startUser) private {
    uint height = _level;
    ...
    if ( referrer != creator && !isReferrer(u) ){
      toHigherPayment(1, referrer, value, _startUser);
      return;
    }
    ...
    //新建一条推广收入记录
    addAccountPromotions(_startUser, referrer, uint128(value));
  }
  ...
  uint constant MAX_FRUIT=1000; //最后的1000个水果
